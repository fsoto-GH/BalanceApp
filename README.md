# Balance App

This is a Personal Balance Tracking App which provides a way to help organize or consolidate your personal expenses.
I created it since I always found myself needing to use a calculator to figure out my finances across different credit cards and banks.

The main purpose of it is to track balances (money you have) against payments (money you owe). However, it does have the ability to accumulate cashback for reference.
From the app, you are able to add, edit, and remove items. You can also export (and import) or save the summary for later use (in receipt form).

# Installation

To get started with BalanceApp, follow these steps:

1. Clone the repository:
   ```bash
   git clone https://github.com/fsoto-GH/BalanceApp.git
   ```
2. Open the solution file in Visual Studio.
3. Build the project to restore dependencies.
4. Run the application using Visual Studio or publish it as an executable.
5. (Optional) You can publish the application with a ClickOnce configuration so that you can easily launch it.

# Screenshots

![A screenshot of my balance application with items.](screenshots/app_view_items.png)
